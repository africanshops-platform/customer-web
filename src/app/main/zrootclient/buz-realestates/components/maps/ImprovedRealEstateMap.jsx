...
// Custom orange marker icon (same brand mark used by the bookings map)
...
function priceSuffix(item) {
  if (item?.propertyUseCase !== "RENT") return "";
  if (item?.leaseTerm === "MONTH") return "/mo";
  if (item?.leaseTerm === "WEEK") return "/wk";
  return "/yr";
}
...
/**
 * ImprovedRealEstateMap Component
 * Enhanced map with better UX using Leaflet with custom tile layers —
 * mirrors ImprovedBookingsMap.jsx so both verticals feel like one product.
 */
function ImprovedRealEstateMap({ items }) {
  const [selectedProperty, setSelectedProperty] = useState(null);

  // Calculate center and bounds from items
  const { center, positions } = useMemo(() => {
    if (!items || items.length === 0) {
      // Default center (Nigeria — Lagos coordinates)
      return {
        ...
        {items
          ...
          .map((item) => (
            <Marker
              ...
              icon={customIcon}
              eventHandlers={{
                click: () => setSelectedProperty(item?.id || item?._id),
              }}
            >
              <Popup
                maxWidth={380}
                ...
                <div className="flex flex-col gap-4 p-3">
                  ...
                  <div className="flex flex-col gap-3">
                    <Typography
                      ...
                      to={`/realestate/listings/${item?.slug}/view`}
                      sx={{
                        fontWeight: 700,
                        fontSize: "1.375rem",
                        color: "#111827",
                    ...
                    {item?.roomCount && (
                      ...
                      >
                        ...
                        {item?.bathroomCount ? ` • 🛁 ${item.bathroomCount} Bath${item.bathroomCount === 1 ? "" : "s"}` : ""}
                      </Typography>
                    )}
...
export default ImprovedRealEstateMap;
